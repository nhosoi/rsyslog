...
processJSON(wrkrInstanceData_t *pWrkrData, smsg_t *pMsg, char *buf, size_t lenBuf)
{
	 ...
	 *  With this configuration, if the json object contains key:value pair 
	 *  "log":"{\"message\":\"Test message\"}", "message":"Test message" is going to
	 *  be added to the top level json with "original_raw_json":"{\"message\":\"Test message\"}".
	...
	if (NULL != pWrkrData->pData->messageField) { /* message_field is configured? */
		struct json_object *str_sub_json;
		/*
		 * Check if the message_field ("log" in this example) exists in the given json,
		 * and the value is the string type.  
		 * E.g., "log":"{\"message\":\"Test message \",\"log_level\":\"INFO\"}"
		 */
		if (json_object_object_get_ex(json, pWrkrData->pData->messageField, &str_sub_json) &&
		    json_object_is_type(str_sub_json, json_type_string)) {
			char *ostr = (char *)json_object_to_json_string_ext(str_sub_json, JSON_C_TO_STRING_PLAIN);
			if (NULL == ostr) {
			...
			if (NULL == nestedjson) {
				DBGPRINTF("mmjsonparse: Error parsing '%s' of '%s' failed\n", ostrcopy, buf);
				json_object_put(json);
				ABORT_FINALIZE(RS_RET_JSON_PARSE_ERR);
			}
